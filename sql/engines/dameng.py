...
class DaMengEngine(EngineBase):
    def get_connection(self, db_name=None):
        ...
        if self.mode == "cluster":
            return dmPython.cluster.DaMengCluster(
                host=self.host,
                port=self.port,
    ...
    def get_all_databases(self, **kwargs):
        ...
        try:
            ...
            logger.warning(f"达梦 CONFIG GET databases 执行报错，异常信息：{e}")
            dbs = [
                int(i.split("db")[1])
    ...
    def query(self, db_name=None, sql="", limit_num=0, close_conn=True, **kwargs):
        ...
        try:
            ...
            logger.warning(f"达梦命令执行报错，语句：{sql}， 错误信息：{traceback.format_exc()}")
    ...
    def execute_workflow(self, workflow):
        ...
        try:
            ...
            logger.warning(
                f"达梦命令执行报错，语句：{cmd or sql}， 错误信息：{traceback.format_exc()}"
            )
            # 追加当前报错语句信息到执行结果中
